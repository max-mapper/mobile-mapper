...
1]},addNew:function(l,s){h.getNext()&&h.clearForward();h.stack.push({url:l,transition:s});h.activeIndex=h.stack.length-1},clearForward:function(){h.stack=h.stack.slice(0,h.activeIndex+1)},ignoreNextHashChange:true},k="[tabindex],a,button:visible,select:visible,input",j=null,o=[],p=false,t="&ui-state=dialog",v=g.children("base"),m=location.protocol+"//"+location.host,r=e.get(m+location.pathname),n=r;if(v.length){var u=v.attr("href");if(u)n=u.search(/^[^:\/]+:\/\/[^\/]+\/?/)===-1?u.charAt(0)==="/"?
m+u:r+u:u;n+=n.charAt(n.length-1)==="/"?" ":"/"}base=a.support.dynamicBaseTag?{element:v.length?v:a("<base>",{href:n}).prependTo(g),set:function(l){base.element.attr("href",n+e.get(l))},reset:function(){base.element.attr("href",n)}}:d;e.setOrigin();a.fn.animationComplete=function(l){if(a.support.cssTransitions)return a(this).one("webkitAnimationEnd",l);else setTimeout(l,0)};a.mobile.updateHash=e.set;a.mobile.path=e;a.mobile.base=base;a.mobile.urlstack=h.stack;a.mobile.urlHistory=h;a.mobile.changePage=
function(l,s,q,w,z){function G(){p=false;o.length>0&&a.mobile.changePage.apply(a.mobile,o.pop())}function Q(){function A(){if(w!==false&&x){h.ignoreNextHashChange=false;e.set(x)}!J&&!L&&h.addNew(x,s);c();a.mobile.silentScroll(l.data("lastScroll"));var H=l,R=H.find(".ui-title:eq(0)");R.length?R.focus():H.find(k).eq(0).focus();y&&y.data("page")._trigger("hide",null,{nextPage:l});l.data("page")._trigger("show",null,{prevPage:y||a("")});a.mobile.activePage=l;M!==null&&M.remove();b.removeClass("ui-mobile-rendering");
G()}function E(H){a.mobile.pageContainer.addClass(H);D.push(H)}a.mobile.silentScroll();var N=f.scrollTop(),K=["flip"],D=[];if(x.indexOf("&"+a.mobile.subPageUrlKey)>-1)l=a("[data-url='"+x+"']");if(y){y.data("lastScroll",N);y.data("page")._trigger("beforehide",{nextPage:l})}l.data("page")._trigger("beforeshow",{prevPage:y||a("")});if(s&&s!=="none"){a.mobile.pageLoading(true);a.inArray(s,K)>=0&&E("ui-mobile-viewport-perspective");E("ui-mobile-viewport-transitioning");if(y)y.addClass(s+" out "+(q?"reverse":
""));l.addClass(a.mobile.activePageClass+" "+s+" in "+(q?"reverse":""));l.animationComplete(function(){l.add(y).removeClass("out in reverse "+s);y&&y.removeClass(a.mobile.activePageClass);A();a.mobile.pageContainer.removeClass(D.join(" "));D=[]})}else{a.mobile.pageLoading(true);y&&y.removeClass(a.mobile.activePageClass);l.addClass(a.mobile.activePageClass);A()}}function S(){if(j||l.data("role")==="dialog"){x=h.getActive().url+t;if(j){l.attr("data-role",j);j=null}}l.page()}var F=a.type(l)==="array",
I=a.type(l)==="object",y=F?l[0]:a.mobile.activePage;l=F?l[1]:l;var x=a.type(l)==="string"?e.stripHash(l):"",B=x,C,O="get",T=false,M=null,P=h.getActive(),J=false,L=false;if(!(P&&h.stack.length>1&&P.url===x&&!F&&!I))if(p)o.unshift(arguments);else{p=true;if(z){a.each(h.stack,function(A){if(this.url===x){urlIndex=A;J=A<h.activeIndex;L=!J;h.activeIndex=A}});if(J){q=true;s=s||P.transition}else if(L)s=s||h.getActive().transition}if(I&&l.url){x=l.url;C=l.data;O=l.type;T=true;if(C&&O==="get"){if(a.type(C)===
"object")C=a.param(C);x+="?"+C;C=d}}base&&base.reset();a(window.document.activeElement).add("input:focus, textarea:focus, select:focus").blur();if(x){l=a("[data-url='"+x+"']");B=e.getFilePath(x)}else{F=l.attr("data-url");I=e.getFilePath(F);if(F!==I)B=I}if(s===d)s=a.mobile.defaultTransition;if(l.length&&!T){B&&base&&base.set(B);S();Q()}else{if(l.length)M=l;a.mobile.pageLoading();a.ajax({url:B,type:O,data:C,success:function(A){var E=a("<div></div>");E.get(0).innerHTML=A;l=E.find('[data-role="page"], [data-role="dialog"]').first();
if(A=E.find("[data-url]").data("url")){base&&base.set(A);x=B=e.getFilePath(A)}else base&&base.set(B);if(!a.support.dynamicBaseTag){var N=e.get(B);l.find("[src],link[href]").each(function(){var K=a(this).is("[href]")?"href":"src",D=a(this).attr(K);D.replace(location.protocol+"//"+location.host+location.pathname,"");/^(\w+:|#|\/)/.test(D)||a(this).attr(K,N+D)})}l.attr("data-url",B).appendTo(a.mobile.pageContainer);S();setTimeout(function(){Q()},0)},error:function(){a.mobile.pageLoading(true);c(true);
base&&base.set(e.get());a("<div class='ui-loader ui-overlay-shadow ui-body-e ui-corner-all'><h1>Error Loading Page</h1></div>").css({display:"block",opacity:0.96,top:a(window).scrollTop()+100}).appendTo(a.mobile.pageContainer).delay(800).fadeOut(400,function(){a(this).remove()});G()}})}}};a("form[data-ajax!='false']").live("submit",function(l){if(a.mobile.ajaxEnabled&&a.mobile.ajaxFormsEnabled){var s=a(this).attr("method"),q=e.clean(a(this).attr("action"));if(!e.isExternal(q)){if(e.isRelative(q))q=
e.makeAbsolute(q);a.mobile.changePage({url:q,type:s,data:a(this).serialize()},d,d,true);l.preventDefault()}}});a("a").live("click",function(l){var s=a(this),q=s.attr("href")||"#";q=e.clean(q);var w=s.is("[rel='external']"),z=e.isEmbeddedPage(q);w=e.isExternal(q)||w&&!z;z=s.is("[target]");var G=s.is("[data-ajax='false']");if(s.is("[data-rel='back']")){window.history.back();return false}if(q==="#")return false;i=s.closest(".ui-btn").addClass(a.mobile.activeBtnClass);if(w||G||z||!a.mobile.ajaxEnabled||
!a.mobile.ajaxLinksEnabled){c(true);if(z)window.open(q);else if(G)return;else location.href=q}else{w=s.data("transition");z=(z=s.data("direction"))&&z==="reverse"||s.data("back");j=s.attr("data-rel");if(e.isRelative(q))q=e.makeAbsolute(q);q=e.stripHash(q);a.mobile.changePage(q,w,z)}l.preventDefault()});f.bind("hashchange",function(){var l=e.stripHash(location.hash),s=a.mobile.urlHistory.stack.length===0?false:d;if(!a.mobile.hashListeningEnabled||!h.ignoreNextHashChange||h.stack.length>1&&l.indexOf(t)>
-1&&!a.mobile.activePage.is(".ui-dialog")){if(!h.ignoreNextHashChange)h.ignoreNextHashChange=true}else l?a.mobile.changePage(l,s,d,false,true):a.mobile.changePage(a.mobile.firstPage,s,true,false,true)})})(jQuery);
